...
class CommunityViewPhotos extends CommunityView
{
	...
	public function editAlbum( $bolSaveSuccess = true )
	{
		...
		$referrer	= $jinput->get->get('referrer' , '', 'STRING'); //JRequest::getVar( 'referrer' , '' , 'GET' );
		...
		if ($bolSaveSuccess === false )
		{
			...
			$album->permissions	= $jinput->post->get('permissions', '', 'NONE'); //JRequest::getVar('permissions', '', 'POST');
	...
	public function album()
	{
		...
		$albumId	= $jinput->get->get('albumid' , 0, 'INT');
 		$defaultId	= $jinput->get->get('photo' , 0, 'INT' );
		$userId	= $jinput->get->get('userid' , 0, 'INT' );
		$user = CFactory::getUser($userId);
		// Set pathway for group photos
...
class CommunityViewPhotosUserHandler extends CommunityViewPhotoHandler
{
	...
	public function __construct( $viewObj )
	{
		...
		$userid			= $jinput->get('userid', null, 'INT');//JRequest::getVar('userid' , '' , 'GET' );
