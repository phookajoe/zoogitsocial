...
class CommunityFilesController extends CommunityBaseController
{
	...
	public function multiUpload()
	{
		...
		$my		= CFactory::getUser();
		$config	= CFactory::getConfig();
		$type	= $jinput->get('type', NULL, 'NONE');	//JRequest::getVar('type');
		...
		if( $my->id == 0 )
		{
			$tokenId	= $jinput->request->get('token' , '', 'STRING');	//JRequest::getVar( 'token' , '' , 'REQUEST' );
			$userId		= $jinput->request->get('uploaderid' , '', 'INT') ; //JRequest::getVar( 'uploaderid' , '' , 'REQUEST' );
    ...
    public function downloadFile()
    {
	    ...
	    $type   = $jinput->get('type','', 'NONE'); //JRequest::getVar('type','');
    ...
    public function ajaxFileDownload($type,$id)
    {
		...
		$objResponse->addScriptCall('window.open(\''.$url.'\');cWindowHide();');
