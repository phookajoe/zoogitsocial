...
<?php
	...
	public function updateAdmin ( $memberId , $groupId, $doAdd = true )
	{
		...
		else
		{
			...
			if($doAdd)
			{
				$response->addScriptCall('joms.jQuery("#member_'. $memberId .' ul li.setAdmin")[0].addClass("cHidden");');
				$response->addScriptCall('joms.jQuery("#member_'. $memberId .' ul li.setAdmin")[1].removeClass("cHidden");');
			}
			else
			{
				$response->addScriptCall('joms.jQuery("#member_'. $memberId .' ul li.setAdmin")[1].addClass("cHidden");');
				$response->addScriptCall('joms.jQuery("#member_'. $memberId .' ul li.setAdmin")[0].removeClass("cHidden");');
			}
	...
	*/
	...
	public function edit()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
			{
				...
				$removeActivity		= $jinput->post->get('removeactivities' , FALSE, 'NONE'); //JRequest::getVar( 'removeactivities' , false , 'POST' );
	...
	public function save()
	{
        ...
        $grouprecentphotos  = $jinput->post->get('grouprecentphotos' , '', 'NONE'); //JRequest::getVar('grouprecentphotos' , '' , 'POST');
        $grouprecentvideos  = $jinput->post->get('grouprecentvideos' , '', 'NONE'); //JRequest::getVar('grouprecentvideos' , '' , 'POST');
        $grouprecentevents  = $jinput->post->get('grouprecentevents' , '', 'NONE'); //JRequest::getVar('grouprecentevents' , '' , 'POST');
			...
			if( $validated )
			{
				...
				if( $isNew )
				{
					...
					if($config->get('groupdiscussnotification') == 1 )
					{
						...
						CNotificationLibrary::add( 'groups_create_discussion' , $discussion->creator , $membersArray , JText::sprintf('COM_COMMUNITY_NEW_DISCUSSION_NOTIFICATION_EMAIL_SUBJECT' , $group->name ) , '' , 'groups.discussion' , $params );
					}
				}
